...
function adjustAllLinesOfText (alignmentType, editor)
{
    ...
    for (; currentRow <= selectionRange.end.row; currentRow++)
    {
        ...
        if (lineIsAcceptableToAdjust (editor.preferredLineLength, currentRowString.length))
            adjustSingleLine (alignmentType, editor, currentRow, currentRowString);

        setSelectionToCoverAllMovedText (editor, selectionRange);
    }
}

function adjustSingleLine (alignmentType, editor, currentRow, currentRowString)
{
    if (alignmentType == "center")
    {
        currentRowString = createPaddingForCenterAlign (editor.preferredLineLength,
    ...
    {
        currentRowString = createPaddingForRightAlign (editor.preferredLineLength,
                                                       currentRowString.length)
                                                     + currentRowString;
    }
    ...
    replaceOldLineWithAdjustedLine (editor, currentRow, currentRowString);
}
